...
def get_latest_version_number() -> str:
    req = requests.get("https://pypi.org/pypi/ohmycloud-fib-py/json")
...
def write_version_to_file(version_number: str) -> None:
    version_file_path: str = str(pathlib.Path(__file__).parent.absolute()) + "/flitton_fib_py/version.py"
    ...
    write_version_to_file(
        version_number=pack_version_number(
            version_buffer=increase_version_number(
                version_buffer=unpack_version_number(version_string=get_latest_version_number())
            )
        )
    )